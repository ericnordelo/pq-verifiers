...
//
// Ported from s2morrow `packages/falcon/src/ntt.cairo` (starkware-bitcoin/s2morrow@831bb518b06d).
// Deviations: dropped upstream's unused `I2_INV` constant (it duplicated the value of `I2`);
// added the negacyclic-wrap and pseudorandom round-trip tests.
...
/// Multiply two polynomials in Z_q[x]/(x^n + 1) using the NTT (negacyclic convolution).
...
mod tests {
    ...
    // Independent of upstream-generated vectors: x · x^511 = x^512 ≡ -1 (mod x^512 + 1),
    // so the product must be the constant polynomial q - 1. Exercises the full
    // NTT → pointwise mul → INTT stack including the negacyclic wraparound.
    #[test]
    fn test_mul_zq_negacyclic_wrap() {
        let mut f: Array<u16> = array![0, 1];
        let mut g: Array<u16> = array![];
        for _ in 2_u32..512 {
            f.append(0);
        }
        for _ in 0_u32..511 {
        ...
        let mut res = mul_zq(f.span(), g.span());
        assert_eq!(res.pop_front().unwrap(), @(Q - 1));
        while let Some(coeff) = res.pop_front() {
            assert_eq!(coeff, @0);
        }
    }

    // Round-trip on a pseudorandom (LCG) vector covering the full [0, Q) range,
    // unlike the monotone [1..n] known-answer inputs.
    #[test]
    fn test_ntt_roundtrip_pseudorandom() {
